...
export default DS.Adapter.extend(Waitable, {
  ...
  _saveHasManyRelationship(store, typeClass, relationship, ids, recordRef, recordCache) {
    ...
    // Added
    var addedRecords = _.filter(ids, (id) => {
      return !idsCache.contains(id);
    });

    // Dirty
    dirtyRecords = _.filter(ids, (id) => {
    ...
    dirtyRecords = _.map(uniq(dirtyRecords.concat(addedRecords)), (id) => {
      return this._saveHasManyRecord(store, typeClass, relationship, recordRef, id);
    });

    // Removed
    var removedRecords = _.filter(idsCache, (id) => {
      return !_.includes(ids, id);
    });

    removedRecords = _.map(removedRecords, (id) => {
  ...
  getFirstEmbeddingParent(internalModel) {
    var embeddingParentRel = _.find(internalModel._implicitRelationships, (implicitRel) => {
   ...
   */
  _queueFlush() {
    _.forEach(this._queue, function WildemberAdapter$flushQueueItem(queueItem) {
      var fn = queueItem[0];
